...
class DQNAgent():
    def __init__(self, env, seed = 42):
        ...
        self.optimizer = optim.AdamW(self.policy_net.parameters(), lr=self.LR, amsgrad=False)
    ...
    def optimize_model_onpolicy(self, state, action, reward, next_state):
        # Compute Q(s_t, a) - the model computes Q(s_t), then we select the
        # columns of actions taken. These are the actions which would've been taken
        # for each batch state according to policy_net
        state_action_values = self.policy_net(state)[0]
        state_action_value = state_action_values[action]
        ...
        if next_state is not None:
            ...
            next_state_value = torch.tensor(0.0).to(device)
    ...
    def optimize_model_single(self):
